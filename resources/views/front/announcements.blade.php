...
<section class="pt-32 px-10 pb-16 bg-gray-700 ">
    <div class="w-max pb-10">
        <h2 class="text-4xl font-medium text-white">Recent announcements</h2>
    </div>
    <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        @foreach($announcements as $announce)
        <li class="h-full bg-gray-800 hover:bg-gray-900 shadow-md hover:shadow-2xl rounded-md overflow-hidden">
            <a href="{{ route('front.show', $announce->id) }}">
                <article class="overflow-hidden rounded-lg shadow-sm transition hover:shadow-lg h-full">


                    <img alt=""
                         src="{{ Str::startsWith($announce->image, 'http') ? $announce->image : asset('storage/' . $announce->image) }}"
                         class="h-44 w-full object-cover"/>
                    <div class=" p-4 sm:p-4 flex flex-col gap-2">
                        <time datetime="" class="block text-xs text-gray-300">
                            {{ $announce->created_at->diffForHumans() }}
                        </time>
                        <h3 class="text-lg font-medium text-white"> {{ $announce->title }}</h3>
                        <p class="text-sm text-gray-400"> {{ $announce->description }}</p>
                    </div>
                </article>
            </a>
        </li>
